...
def _chat_system_prompt(allowed_types: frozenset[ChatFactType]) -> str:
    ...
    return (
        ...
        "   references it via [^N].\n"
        "6. Patient-reported answers from intake forms must use "
        "   type='intake_answer' and cite QuestionnaireResponse rows. Do "
        "   not relabel those answers as medication, allergy, demographics, "
        "   or problem facts unless you also cite the corresponding "
        "   structured chart resource from a chart tool.\n"
        "7. General clinical guideline claims must use type='guideline' and "
        "   cite ClinicalGuidelineChunk rows. Guideline rows are public "
        "   evidence, not patient chart evidence; do not use them to claim "
        "   the patient has a condition, medication, lab value, or history.\n"
        "\n"
        "tool use:\n"
        ...
        "  get_immunizations; lab questions (whether the result was "
        "  clinician-entered, HL7-vendor, or AI-extracted from an uploaded "
        "  PDF) require get_lab_trend or get_observations — both surface "
        "  AI-extracted rows alongside structured rows, with AI rows "
        "  carrying an aiProvenance field for source citation; "
        "  intake-form / questionnaire / patient-reported answer questions "
        "  require get_questionnaire_responses — AI-extracted intake forms "
        "  appear here too, with each item carrying its own aiProvenance "
        "  for the answer it represents; "
        "  clinical guideline, screening, immunization, preventive-care, "
        "  counseling, pharmacology, or public-health guidance questions "
        "  require search_clinical_guidelines; "
        ...
        "- QuestionnaireResponse rows from get_questionnaire_responses carry "
        "  the patient's intake-form answers. Each item's aiProvenance.snippet "
        "  is the verbatim source text; cite item.text or item.answer with "
        "  type='intake_answer'.\n"
        "- For AI-extracted rows (Observation or QuestionnaireResponse), "
        "  copy dates and numbers exactly as they appear in the cited row "
        "  or in aiProvenance.snippet. Do not normalize a date like "
        "  8/14/1967 into 1967-08-14 unless that exact ISO date is in the "
        "  cited row.\n"
        "- If the user asks what changed, what to pay attention to, or what "
        "  evidence supports a finding, get_observations and get_lab_trend "
        "  surface AI-extracted lab rows (distinguishable by aiProvenance), "
        "  and get_questionnaire_responses surfaces AI-extracted intake "
        "  answers (each item with its own aiProvenance). Cite "
        "  aiProvenance.snippet/page for source attribution.\n"
