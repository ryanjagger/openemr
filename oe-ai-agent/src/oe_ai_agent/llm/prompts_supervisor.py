...
def _extractor_system_prompt() -> str:
    return (
        ...
        "\n"
        "Rules:\n"
        "- Always extract any unindexed lab_report when the user's question "
        "  is about labs, observations, vitals, or any clinical measurement. "
        "  Always extract any unindexed intake_form when the user's question "
        "  is about patient-reported information, history, intake answers, "
        "  questionnaire responses, social history, allergies the patient "
        "  recalled, or medications the patient listed. You CANNOT tell "
        "  from a filename alone what's inside the PDF — a doc named "
        "  'cbc.pdf' might still help answer a lipid-panel question, and a "
        "  doc named 'intake.pdf' might list medications. When in doubt for "
        "  a relevant document_type, extract.\n"
        ...
        "- Only skip extraction when the unindexed document_type is "
        "  unrelated to the user's question category (e.g., user asks about "
        "  appointments and only an intake_form is unindexed). In that "
        "  case, respond with a single sentence saying so and call no tools.\n"
    )
...
def _evidence_system_prompt(allowed_types: frozenset[ChatFactType]) -> str:
    ...
    return (
        ...
        "- BROADEN ON ZERO: when your specific-term lab queries (e.g. "
        "  get_lab_trend('lipid panel'), get_observations('LDL', "
        "  category='laboratory')) all return zero rows, do not stop. The "
        "  chart may contain labs that don't match the user's exact search "
        "  terms — for example, an uploaded lab report could be a CBC when "
        "  the user asked about a lipid panel. Before finalizing, call "
        "  get_observations(category='laboratory') with NO code_or_text "
        "  filter (or get_recent_observations) to surface what labs "
        "  actually exist. Same applies to intake/history questions: if "
        "  specific get_questionnaire_responses queries return zero, call "
        "  get_questionnaire_responses with no filter to see what intake "
        "  answers exist.\n"
    )
